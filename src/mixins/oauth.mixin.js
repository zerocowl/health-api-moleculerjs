  ...
  methods: {
    ...
    async getUser(username, password) {
      ...
      try {
        ...
        if (!user) throw new MoleculerError(`Usuário não encontrado, ${username}`, 404);
        if (!user.password) throw new MoleculerError('Usuário não possui senha', 400);
        if (!user.verifyPassword(password)) {
          throw new MoleculerError('Senha incorreta', 400);
        }
        return user;
      } catch (err) {
        this.logger.error(err);
